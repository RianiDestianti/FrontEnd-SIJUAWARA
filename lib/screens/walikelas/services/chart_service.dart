...
class ChartCredentials {
  ...
  const ChartCredentials({required this.teacherClassId, required this.nipWalikelas});
...
class ChartService {
  ...
  static Future<List<ChartDataItem>> fetchChartData({
    ...
    final params = {'nip': creds.nipWalikelas, 'id_kelas': creds.teacherClassId};

    var response = await ApiClient.get(primaryEndpoint, params: params);
    if (response.statusCode != 200 && fallbackEndpoint != null) {
      final retry = await ApiClient.get(fallbackEndpoint, params: params);
    ...
    if (rawList.isEmpty) throw Exception(jsonData['message'] ?? 'Gagal memuat data');
