  ...
  class HomeScreenState extends State<HomeScreen> {
    ...
    Widget build(BuildContext context) {
      return Scaffold(
        backgroundColor: const Color(0xFFF8FAFC), // ✅ back to gray, NOT blue
        extendBody: true,
  ...
  class HomeScreenContentState extends State<HomeScreenContent>
    ...
    void filterSiswa(String query) {
      ...
      if (query.isNotEmpty) {
        addLocalActivity('Pencarian', 'Pencarian Siswa', 'Melakukan pencarian: $query');
      }
    }
    ...
    void navigateToDetailScreen(Student siswa) {
      Navigator.push(
        ...
        addLocalActivity('Navigasi', 'Detail Siswa', 'Mengakses detail siswa: ${siswa.name}');
      });
    }
    ...
    Widget build(BuildContext context) {
      ...
      final apresiasiChartData = ChartUtils.aggregateChartData(
        apresiasiRawData, apresiasiChartTab,
      );
      final pelanggaranChartData = ChartUtils.aggregateChartData(
        pelanggaranRawData, pelanggaranChartTab,
      );
                                    ...
                                    children: [
                                      if (selectedTab == 2) ...[
                                        SectionWidgets.buildCompactSiswaTerbaikSection(
                                          ...
                                          filteredSiswaTerbaik, navigateToDetailScreen,
                                        ),
                                      ] else if (selectedTab == 3) ...[
                                        SectionWidgets.buildCompactSiswaBeratSection(
                                          isSmallScreen, cardPadding,
                                          filteredSiswaBerat, navigateToDetailScreen,
                                        ),
                                      ] else ...[
                                        SectionWidgets.buildCompactChartCard(
                                          ...
                                          ChartWidgets.buildBarChart(apresiasiChartData,
                                            const LinearGradient(colors: [Color(0xFF61B8FF), Color(0xFF0083EE)])),
                                          apresiasiChartTab,
                                          (index) => setState(() => apresiasiChartTab = index),
                                          true, isSmallScreen, cardPadding,
                                          ChartWidgets.buildSwipeableChartButtons(apresiasiChartTab,
                                            (index) => setState(() => apresiasiChartTab = index),
                                            addLocalActivity),
                                          () => Navigator.push(context, MaterialPageRoute(
                                            builder: (context) => GrafikScreen(
                                              chartType: 'apresiasi',
                                              title: 'Grafik Apresiasi Siswa',
                                              subtitle: 'Pencapaian positif',
                                            ))),
                                        ),
                                        ...
                                        SectionWidgets.buildCompactChartCard(
                                          ...
                                          ChartWidgets.buildBarChart(pelanggaranChartData,
                                            const LinearGradient(colors: [Color(0xFFFF6B6D), Color(0xFFFF8E8F)])),
                                          pelanggaranChartTab,
                                          (index) => setState(() => pelanggaranChartTab = index),
                                          false, isSmallScreen, cardPadding,
                                          ChartWidgets.buildSwipeableChartButtons(pelanggaranChartTab,
                                            (index) => setState(() => pelanggaranChartTab = index),
                                            addLocalActivity),
                                          () => Navigator.push(context, MaterialPageRoute(
                                            builder: (context) => GrafikScreen(
                                              chartType: 'pelanggaran',
                                              title: 'Grafik Pelanggaran Siswa',
                                              subtitle: 'Monitoring pelanggaran',
                                            ))),
                                        ),
                                        SizedBox(height: sectionSpacing),
                                        SectionWidgets.buildCompactActivityCard(
                                          isSmallScreen, cardPadding, activityData,
                                          () => Navigator.push(context, MaterialPageRoute(
                                            builder: (context) => const ActivityScreen())),
                                          (activity, isSmall) => SectionWidgets.buildCompactActivityItem(
                                            activity, isSmall,
                                            () => Navigator.push(context, MaterialPageRoute(
                                              builder: (context) => const ActivityScreen()))),
                                        ),
    ...
    Widget _buildHeader(bool isSmallScreen, double headerPadding) {
      return Container(
        decoration: const BoxDecoration(
          ...
          // ✅ No top radius needed — SafeArea already clips the status bar
          // and the outer Container is the same blue color
        ...
        child: Padding(
          padding: EdgeInsets.fromLTRB(
            headerPadding,
            isSmallScreen ? 12 : 16, // ✅ No statusBarHeight needed — SafeArea handles it
            headerPadding,
            isSmallScreen ? 20 : 24,
          ),
          child: Column(
              ...
              Row(
                children: [
                  HeaderWidgets.buildActionButton('Umum', 0, selectedTab, () {
                    ...
                    addLocalActivity('Navigasi', 'Tab Umum', 'Berpindah ke tab Umum');
                  }),
